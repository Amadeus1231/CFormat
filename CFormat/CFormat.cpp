...
TApcHttpParser::TApcHttpParser(enHttpDataType abType)
...
{
  int nParsed = http_parser_execute(&m_Parser, &m_ParsSettings,
                                    reinterpret_cast<const char*>(apBinBuf),
                                    anBufSize);
  if (nParsed != anBufSize) {
    __L_BAD(m_pLog, "Num of processed not equal to putted. Income bytes: "
                        + apc::toStr<int>(anBufSize)
                        + ", parsed: " + apc::toStr<int>(nParsed));
    return -33;
  }
...
int TApcHttpParser::getFieldByID(const std::string& astrFieldID,
...
{
  std::unique_ptr<TApcUnorderedMap<std::string, std::string>> spHeaders(
      new TApcUnorderedMap<std::string, std::string>());
  std::string strKey, strVal;
  for (int nRes = m_Fields.getFirstPair(strKey, strVal); !nRes;
...
int TApcHttpParser::onField(const char* aszInfo, int anLength)
{
  ...
  if (m_bState != 1) {
    if (m_strCurrFieldID != "") {
      __L_BAD(m_pLog,
              "m_strCurrFieldID is not nullptr. Value: " + m_strCurrFieldID);
    }
...
int TApcHttpParser::onResStatus(const char* aszInfo, int anLength)
{
  ...
  if (m_bState != 3) {
    if (m_strCurrResStatus != "") {
      __L_BAD(m_pLog, "m_strCurrResStatus is not nullptr. Value: "
                          + m_strCurrResStatus);
    }
...
void TApcHttpParser::finalizePreSection(BYTE abCurrState)
{
  switch (abCurrState) {
    ...
    case 2:
    {
      ...
      // Всё будем хранить в "верхнем регистре", чтобы потом искать "без
      // регистра"
      ...
      if (nResult) {
        if (strUpperCaseField == "VARY") { // 'Vary' exception
          if (m_Fields.set(strUpperCaseField, m_strCurrValueID)) {
            __L_BADR(m_pLog,
                     "Error in updatePair() for: 'VARY' value: '"
                         + m_strCurrValueID + "'",
                     nResult);
          }
        }
          ...
          __L_BADR(m_pLog,
                   "Error in addPair() for: " + strUpperCaseField
                       + ", value: " + m_strCurrValueID
    ...
    default:
      __L_BAD(m_pLog,
              "Unexpected state. Value: " + apc::toStr<BYTE>(abCurrState));
      break;
  };
};
//---------------------------------------------------------------------------
...
int TApcHttpParser::onHeaderComplete()
{
  ...
  if (m_Parser.flags & F_CONTENTLENGTH) {
    __L_ANY(m_pLog, "Size of content payload: "
                        + apc::toStr<uint64_t>(m_Parser.content_length))
  }
  else {
    __L_ANY(m_pLog, "No contents payload");
  }

  // Если есть длина в заголовке, или же проставлен флаг chuncked, то
  // продолжаем, иначе - выходим
  if (!(m_Parser.flags & F_CONTENTLENGTH) && !(m_Parser.flags & F_CHUNKED)) {
    if (m_bParseType
        == enHttpDataType::enHttpDataType_Response) // на "ответ" будем
                                                    // считать что мы всё.
    {
      m_fIsCompelte = true;
    }
...
int TApcHttpParser::onMsgBegin()
{
  ...
  m_strCurrFieldID = m_strCurrValueID = m_strCurrResStatus = m_strCurrBody
      = m_strCurrURL = "";
  m_Fields.clear();
...
int TApcHttpParser::onBody(const char* aszInfo, int anLength)
{
  ...
  if (m_bState != 4) {
    if (m_strCurrBody != "") {
      __L_BAD(m_pLog,
              "m_strCurrBody is not nullptr. Value: " + m_strCurrBody);
    }
...
std::string TApcHttpParser::printFlags(BYTE abFlags)
{
  ...
  // TODO: добавить вывод оставльных флагов? Или это вообще нигде не
  // используется?
